...
/* This function was described in pseudocode by trincot on Stackoverflow at: 
 https://stackoverflow.com/questions/46695942/iterative-in-place-mergesort
 This pseudocode was then used in partnership with geeksforgeeks.org (URL listed below)
 in order to develop the following function. Small changes were made, including passing the
 'mid+1' variable (renamed L2) into the function itself rather than calculating it within 
 the function. The geeksforgeeks page describes a recursive implementation of an in-place
 merge sort, so it was modified for an interative implementation.
 While the outline was taken from online sources, Detailed comments describe our understanding of the processes. */
function merge(array,L1,R1,L2,R2){      
    ...
    while (L1 <= R1 && L2 <= R2) {          // This will run as long as the array length is > 1
        if (array[L1] <= array[L2]) {       // If the first element is smaller, only increment the bounds
            ...
            // The two lines of code below are partly what allows us to make this function in-place
            ...
            /* Language received from geekforgeeks.org: Shift all elements between element1 
            and element 2 right by 1 (including element1 but excluding element2) and then 
            place the element2 in the previous position of element 1 (before shifting right) */
